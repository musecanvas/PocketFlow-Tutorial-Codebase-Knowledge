...
docker run -it --rm \
  ...
  pocketflow-app --dir /app/code_to_analyze --language "Chinese"
...
# 5. **`-v "$HOME/Documents/java_project/j2pfg_vscode":/app/code_to_analyze`**：
#    （核心步驟）將你本機的 Java 專案原始碼掛載到容器內的 `/app/code_to_analyze` 目錄，這樣 AI 工具才能讀取並分析這些程式碼。
...
# 8. **`--dir /app/code_to_analyze --language "Chinese"`**：
